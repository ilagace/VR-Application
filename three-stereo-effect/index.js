...
module.exports = function(THREE, renderer) {
	...
	function StereoEffect(renderer){
		...
		this.render = function ( scene, camera ) {
			...
			var _cameraL2 = camera;
			var _cameraR2 = camera;
			_cameraR2.translateX( this.eyeSeparation / 2.0 );
			_cameraL2.translateX( - this.eyeSeparation / 2.0 );
            if (firstpass) {console.log(_cameraR.position,this.eyeSeparation); firstpass = false;}
			...
			renderer.render( scene, _cameraL2 );

			renderer.setScissor( _width, 0, _width, _height );
			renderer.setViewport( _width, 0, _width, _height );
			renderer.render( scene, _cameraR2 );
